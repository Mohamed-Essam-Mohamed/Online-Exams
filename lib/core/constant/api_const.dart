abstract class ApiConst {
  static const String baseUrl = 'https://exam.elevateegy.com/';
  static const String login = 'api/v1/auth/signin';
  static const String register = 'api/v1/auth/signup';
  static const String changePass = 'api/v1/auth/changePassword';
  static const String deleteAccount = 'api/v1/auth/deleteMe';
  static const String logout = 'api/v1/auth/logout';
  static const String profileData = 'api/v1/auth/profileData';
  static const String forgetPass = 'api/v1/auth/forgotPassword';
  static const String verifyResetCode = 'api/v1/auth/verifyResetCode';
  static const String resetPass = 'api/v1/auth/resetPassword';
}
